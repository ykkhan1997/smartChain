const { keccakHash } = require("../util");
const Trie=require("./trie");

describe("Trie",()=>{
    let trie;
    beforeEach(()=>{
        trie=new Trie();
    });
    describe("rootHash()",()=>{
        it("check that the rootHash is not undefined",()=>{
            expect(trie.rootHash).not.toBe(undefined);
        });
    });
    describe("put()",()=>{
        it("store a value under a key",()=>{
            const key="foo";
            const value="bar";
            trie.put({key,value});
            expect(trie.get({key})).toEqual(value);
        });
        it("generate a new rootHash after entering the value",()=>{
            const originalRootHash=trie.rootHash;
            const key="foo";
            const value="bar";
            trie.put({key,value});
            expect(originalRootHash).not.toEqual(trie.rootHash);
        });
    });
    describe("get()",()=>{
        it("returns a copy of the stored value",()=>{
            const key="foo";
            const value={one:1};
            trie.put({key,value});
            const gottenValue=trie.get({key});
            value.one=2;
            expect(gottenValue).toEqual({one:1});
        });
    });
    describe("buildTrie()",()=>{
        it("build a trie where the items are accessible with their hashes",()=>{
            const item1={"foo":"bar"};
            const item2={"foo1":"bar1"};
            trie=Trie.buildTrie({items:[item1,item2]});
            expect(trie.get({key:keccakHash(item1)})).toEqual(item1);
        })
    })
});